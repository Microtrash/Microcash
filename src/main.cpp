...
int64 nChainStartTime = 1387438382;
...
int64 GetProofOfWorkReward(unsigned int nHeight)
{
		...
		if (nHeight < 11)
			nSubsidy = 1200000 * COIN; // 12 million premine. 1.2mil each for the 10 "Trusted nodes" ;)
		else 
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime)
{
    ...
    if(fTestNet || nTime > PROTOCOL_SWITCH_TIME)
    {
        ...
        // Microcash: reward for coin-year is cut in half every 64x multiply of PoS difficulty
...
//vMerkleTree: ea6fed5e2
        ...
        assert(block.hashMerkleRoot == uint256("0x7062f9ce7e4edec4ea5f3f5d0590fd9d6bad8cf5b73d563c84f6daef7052cd9e"));
